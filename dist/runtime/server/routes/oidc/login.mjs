...
export default defineEventHandler(async (event) => {
  ...
  const defCallBackUrl = getDefaultBackUrl(
    redirectUrl,
    req.headers.host,
    op.useSsl
  );
  ...
  logger.info(
    "[Login]: cabackurl & op.callbackUrl & redirecturl: ",
    callbackUrl,
    op.callbackUrl,
    redirectUrl
  );
  logger.info(
    "  response_mode:" + responseMode + ", response_type:" + config.response_type + ", scopes:" + scopes.join(" ")
  );
  const parameters = {
